...
Use clear imperative commit titles (e.g., `feat: expose run_pipeline helper`). Include context on schema/SQL changes, sample CLI invocation, and any new env vars. When plots change, attach images; otherwise share the relevant S3 URLs or CSV diffs. Reference associated tickets to keep traceability.
